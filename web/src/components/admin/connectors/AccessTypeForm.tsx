...
export function AccessTypeForm({
  ...
  options.push({
    ...
    value: "public",
    description:
      "Everyone with an account on HaqqAI can access the documents pulled in by this connector",
    disabled: false,
    disabledReason: "",
  });
  ...
  if (showAutoSync) {
    options.push({
      ...
      value: "sync",
      description:
        "We will automatically sync permissions from the source. A document will be searchable in HaqqAI if and only if the user performing the search has permission to access the document in the source.",
